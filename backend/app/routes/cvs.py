...
async def upload_cv(
    ...
    if current_user.role == "ADMIN":
        ...
        valid_tier = db.query(Tier).filter(Tier.name == tier).first()
        if not valid_tier:
        ...
        if tier and tier != current_user.role:
             ...
             if not can_access_tier(current_user.role, tier, db):
...
def get_cv(cv_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not can_access_tier(current_user.role, cv.tier, db):
